...
- dock-driven relative navigation now reuses the same viewport-alignment path after opening the next or previous song, so the active item stays visible without forcing the list `selected` state to collapse back onto the active song
