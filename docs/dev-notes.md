...
* [assumption] PDF preview fit is decided from the actual app viewer surface, prefers height-first page visibility by default, and only uses side-by-side pages when the available width can support them without sacrificing the primary reading fit
  impact: medium
  revisitable: yes
* [assumption] the current Performance reading surface uses ChordPro CLI-generated HTML from the current `.cho` as the source of truth instead of the internal `Song` model, keeps PDF out of Performance mode, and favors adaptive reading controls over PDF fidelity
  impact: medium
  revisitable: yes
...
* [assumption] `Quality` / `Fast` should remain the primary user-facing conversion modes even if model selection becomes hierarchical underneath, and any automatic fallback should stay within the same mode with explicit user feedback rather than silently crossing between modes or exposing low-level model toggles by default
  impact: medium
  revisitable: yes
* [assumption] future Performance reader improvements should continue from the CLI-HTML reader path unless real-use validation shows the CLI markup blocks needed behavior; likely next refinements include auto-fit zoom, reader-specific chord diagrams and richer section navigation while keeping PDF as the reference/export output outside Performance
  impact: medium
  revisitable: yes
...
- Performance mode still has room for UX refinement, but those changes should be handled through a structured UX review rather than ad-hoc incremental tweaks; the current `v1.7` interaction direction is a direct active-song navigation model with immediate UI feedback independent of preview completion, a secondary song list used as a visual index, a dual `full preview` / `split list + preview` layout strategy, and render debounce applied to preview refresh rather than to navigation itself

## Post global usage review (v1.4.x stabilization)

- the keyboard vs mouse selection conflict in Songbook is resolved by tracking the last input source locally in the parent view, so passive hover no longer overrides keyboard-driven selection during autoscroll; Performance no longer uses that persistent browsing selection model and instead treats hover/focus feedback as temporary
...
- the shared PDF.js viewer is used by Convert, Songbook and Playground; it provides app-styled controls, height-first default fit, continuous page rendering, page navigation, zoom/reset controls and drag-to-pan without changing preview cache or export behavior
- standard preview surfaces are now app-owned enough that clicking, scrolling or panning inside the PDF document must not steal keyboard ownership from the surrounding app surface; this fixes the old native-PDF focus trap where Songbook shortcuts stopped responding until focus returned to non-viewer UI
- Performance mode now uses a separate app-owned HTML reader generated by the ChordPro CLI from the current `.cho`, cached separately from PDF previews, sanitized in the frontend, and adapted for live reading with fixed title/artist context, reader font controls, adaptive 3 / 2 / 1 column fit and continuous scrolling fallback
- the Performance HTML reader keeps ChordPro's table-based chord/lyric alignment but avoids adding cell padding that would create artificial word or syllable gaps; narrow/tall layouts reserve left reader space for floating song navigation controls instead of letting those controls cover lyrics or chords
...
- Songbook performance mode in `User` now switches the app shell into an immersive low-padding layout, keeps navigation controls local to the view, and uses the dedicated Performance HTML reader instead of the standard PDF preview surface
- keyboard navigation in Performance mode remains app-owned around the song list, reader surface and floating controls; clicking, scrolling or otherwise interacting with the reader must not trap focus in a viewer sub-surface or block shortcuts such as `Esc`, previous song and next song
...
- `v1.7` performance-mode interaction now uses an `active`-only song model: click, `Enter`, `Space`, `ArrowUp`, `ArrowDown` and the floating previous/next controls all load or move the active song directly, the list remains open during direct song loading, hover/focus feedback is temporary, and the parent Songbook selection is only synchronized to the active song for orientation when returning to Songbook
- dock-driven relative navigation now reuses the same viewport-alignment path after opening the next or previous song, so the active item stays visible without maintaining a separate persistent Performance list selection
...
- Songbook list interaction in the main `User` view keeps the fuller browsing model: `active` stays tied to the song currently opened in preview, `selected` tracks keyboard or mouse browsing, hover can update selection without opening the song, and the list container keeps keyboard ownership so `ArrowUp` / `ArrowDown` / `Enter` / `Space` behave consistently even after focus leaves the list
- view transitions between Songbook and Performance preserve Songbook's local `selected` state where useful, while Performance now recenters on the current `active` song and treats hover/focus feedback as temporary rather than as a persistent list selection
